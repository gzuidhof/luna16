    ...
    if P.AUGMENT and P.AUGMENTATION_PARAMS['flip']:
        ...
        for filename, image, label in result:
            if label == 1:
                ...
                flipped_images = flipped_images[:3] #SELECT 3 RANDOM IMAGES OF 7 possible flips
...
def make_epoch(n, train_true, train_false, val_true, val_false):
    ...
    train_epoch = train_true + train_false[:n_train_true*4] #*4 to account for 3 flip directions
    val_epoch = val_true + val_false[:n_val_true*4]
